...
Cypress.Commands.add("createNewStory", () => {
  ...
  cy.window().then((win) => {
    console.log(win.top[0].eidosmedia);
    const eidosmedia = win.top[0].eidosmedia;
    window.top.eidosmedia = eidosmedia;
  })
  cy.wait(5000);
  cy.frameLoaded('iframe[id="editor_1_ifr"]');
  // cy.then(() => {                    // put the call on the Cypress queue 
  //   // to get the timing right

  //   cy.wait(2000)
  //   getIframeBody().find("div[eom-name='headline']", { timeout: 2000 }).should('be.visible');
  //   cy.wait(2000)
  //   getIframeBody().find("div[eom-name='headline']", { timeout: 6000 }).type("Creating Sample headline text")
  // })
  // cy.url().then(url => {
  //   const getUrl = url;
